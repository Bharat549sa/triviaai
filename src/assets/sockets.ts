...
// const TranslateCode = (uri: string) => {
//     try {
...
const code = (): string => {
  // Create a random string of 6 characters (a simple example).
  return Math.random().toString(36).substr(2, 6).toUpperCase();
};

const TranslateCode = (code: string): string => {
  // Example: Adding a prefix or processing the code in some way.
  return `PREFIX-${code}`;
};
...
export function io(uri: string): Promise<Socket> {
  return new Promise((resolve, reject) => {
    ...
    peer.on("open", (id) => {
      ...
      const dataConnection = peer.connect(TranslateCode(uri) as string, {
        reliable: true,
      });
...
export class Server {
  ...
  constructor(
    ...
    while (error) {
      try {
        _code = code();
        _socket = new Peer(TranslateCode(_code) as string, {
          debug: 0,
          secure: true,
        });
        error = false;
